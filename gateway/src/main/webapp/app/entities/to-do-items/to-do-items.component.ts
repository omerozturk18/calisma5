...
export class ToDoItemsComponent implements OnInit, OnDestroy {
  ...
  loadAll() {
    this.toDoItemsService
      ...
      .subscribe(
        (res: HttpResponse<IToDoItems[]>) => this.paginateToDoItems(res.body, res.headers),
  ...
  trackId(index: number, item: IToDoItems) {
    return item.id;
  }

  registerChangeInToDoItems() {
    this.eventSubscriber = this.eventManager.subscribe('toDoItemsListModification', response => this.reset());
  }
  ...
  protected paginateToDoItems(data: IToDoItems[], headers: HttpHeaders) {
